...
class TestInMemoryCheckpointStore:
    ...
    async def test_list_checkpoints_with_prefix(self, store):
        ...
        await store.save(
            "project-a-wf-1", create_workflow_state("A1", "project-a-wf-1")
        )
        await store.save(
            "project-a-wf-2", create_workflow_state("A2", "project-a-wf-2")
        )
        await store.save(
            "project-b-wf-1", create_workflow_state("B1", "project-b-wf-1")
        )
