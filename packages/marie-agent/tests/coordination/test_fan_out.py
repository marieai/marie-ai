...
class TestFanOutBasicExecution:
    ...
    async def test_run_single_agent(
        self, fan_out_coordinator, mock_agent, sample_messages
    ):
    ...
    @pytest.mark.asyncio
    async def test_run_multiple_agents(
...
class TestFanOutConcurrency:
    ...
    async def test_parallel_execution_faster_than_sequential(self, sample_messages):
        ...
        delay = 0.1
        agents = [SlowMockAgent(name=f"agent_{i}", delay=delay) for i in range(3)]
        coordinator.add_agents(agents)
