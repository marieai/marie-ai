...
- **Processing visitors:** Classes or instances (implementing `ProcessingVisitor`) used to traverse, extract, or modify the execution context.
...
The coercers ensure uniform call semantics regardless of function/class form.
...
- Automatically scans for submodules with prefixes: `parsers`, `validators`, `template_builders`, and `processing_visitors`.
...
- `register_processing_visitor(name)`
...
- **Processing visitors:** duplicate names log a warning and overwrite.
...
Register a processing visitor (class form):
```python
from marie.extract.registry.base import register_processing_visitor
from marie.extract.engine.base import BaseProcessingVisitor

@register_processing_visitor("custom_audit_visitor")
class CustomAuditVisitor(BaseProcessingVisitor):
    def pre_visit(self, context, parent):
        print("Starting audit...")

    def visit(self, context, parent):
        # custom logic
        pass
        
    def post_visit(self, context, parent):
        pass
```
