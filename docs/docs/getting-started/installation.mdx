...
### Troubleshooting Python 3.12 debugging
Debugging code with multiprocessing manager with Python 3.12

[https://youtrack.jetbrains.com/issue/PY-66008/SystemError-debugging-code-with-multiprocessing-manager-with-Python-3.12#steps-to-reproduce] (https://youtrack.jetbrains.com/issue/PY-66008/SystemError-debugging-code-with-multiprocessing-manager-with-Python-3.12#steps-to-reproduce)

## Infrastructure Services

Marie-AI uses several infrastructure services for analytics, storage, and integrations. These services can be deployed via Docker Compose or Kubernetes.

### ClickHouse

[ClickHouse](https://clickhouse.com/) is a column-oriented database used by marie-ai for high-performance analytics and job metrics.
...
The `marie` database is required for LLM tracking and analytics. If using the bootstrap script, it will be created automatically:

```bash
./bootstrap-marie.sh --start
```

To create the database manually:
```bash
docker exec marie-clickhouse clickhouse-client --query "CREATE DATABASE IF NOT EXISTS marie"
```

Verify the database exists:
```bash
docker exec marie-clickhouse clickhouse-client --query "SHOW DATABASES"
```

### Gitea

[Gitea](https://gitea.com/) is a self-hosted Git service used by marie-studio for repository management and version control.
...
On first access to `http://localhost:3001`, Gitea displays an installation wizard. The database connection is pre-configured via environment variables to use shared PostgreSQL.
:::

### Full Stack Deployment

Deploy all infrastructure services together:
...
For Kubernetes deployment, see the [Kubernetes deployment guide](/docs/getting-started/deployment/kubernetes).
