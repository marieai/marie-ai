...
def generate_alpha_numeric(length: int, alpha: bool = True, numeric: bool = True) -> str:
    ...
    """
    assert alpha or numeric, "Cannot generate string: param alpha or numeric must be True"
    return fake.password(length=length, digits=numeric, upper_case=alpha,
...
def create_annotation_data(text: str, width: int, height: int, font_path: str, dec: int = 2):
    """
...
def _augment_decorated_process(
    ...
    for k in range(count):
        ...
        for item in data["form"]:
            ...
            words_annotations, line_coordinates, font_size = create_annotation_data(aug_text, w, h, font_path)
...
def augment_decorated_annotation(count: int, src_dir: str, dest_dir: str, mask_config_path: str):
    ...
    for guid, file in enumerate(os.listdir(ann_dir)):
        ...
        _args = (count, file_path, img_path, font_dir, dest_aug_annotations_dir, dest_aug_images_dir, mask_config)
...
def get_augmenter_parser(subparsers=None) -> argparse.ArgumentParser:
    ...
    if subparsers is not None:
        augmenter_parser = subparsers.add_parser(
            "augment", help="Creates a number of augmented images and annotation from a source dataset."
        )
    else:
        augmenter_parser = argparse.ArgumentParser(
            prog="augmenter", description="Creates a number of augmented images and annotation from a source dataset."
        )
