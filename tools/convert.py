...
def _validate_qa_balance(category_counts: dict, link_map: dict, id_to_name: dict) -> dict:
...
def _convert_coco_to_funsd(
    ...
    category_imbalance = _validate_qa_balance(annotation_category_counts, link_map, config_id_to_name)
    ...
    if len(category_imbalance) > 0:
        ...
        for image_id, annotations in annotations_by_image.items():
            ...
            imbalance = _validate_qa_balance(image_category_counts, link_map, config_id_to_name)
            if len(imbalance) > 0:
...
def convert_coco_to_funsd(
    ...
    items = glob.glob(os.path.join(src_dir, "annotations/*.json"))  # instances_default.json
    if len(items) == 0:
...
def get_convert_parser(subparsers=None) -> argparse.ArgumentParser:
    ...
    if subparsers is not None:
        ...
        convert_parser = argparse.ArgumentParser(prog="convert", description="COCO to FUNSD conversion")
