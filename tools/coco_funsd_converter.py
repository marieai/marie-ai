...
def __convert_coco_to_funsd(
    ...
    category_imbalance = __validate_qa_balance(
    ...
    if len(category_imbalance) > 0:
        ...
        for image_id, annotations in annotations_by_image.items():
            ...
            imbalance = __validate_qa_balance(
...
def extract_icr(image, label: str, boxp, icrp, debug_fragments: bool = False):
    ...
    file_dir = ensure_exists(f"{_tmp_path}/icr/{checksum}/")
    ...
    if boxes is None or len(boxes) == 0:
        ...
        if debug_fragments:
            file_path = os.path.join(file_dir, f"{checksum}.png")
    ...
    for item in data["form"]:
        ...
        if debug_fragments:
            file_path = os.path.join(
                f"{_tmp_path}/snippet", f"{filename}-snippet_{_id}.png"
            )
...
def generate_money(
    num_digits: int,
    decimal_place_buffer: int = 2,
    neg_prob: float = 0.2,
    sign_prob: float = 0.5,
    dec_only_prob: float = 0.2,
) -> str:
...
def generate_alpha_numeric(
    original: str, alpha: bool = True, numeric: bool = True
) -> str:
...
def __augment_decorated_process(
    ...
    for k in range(count):
        ...
        for item in data["form"]:
            ...
            words_annotations, line_coordinates, font_size = generate_annotation_data(
...
def augment_decorated_annotation(
    count: int, src_dir: str, dest_dir: str, font_dir: str
):
...
def default_augment(args: object):
    ...
    augment_decorated_annotation(
        count=aug_count, src_dir=src_dir, dest_dir=dst_dir, font_dir=font_dir
    )
