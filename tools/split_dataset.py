...
def split_coco_annotation_file(train: list, test: list, annotation_path: str):
    ...
    images = {os.path.split(image["file_name"])[1]: image for image in coco_annotation_file.pop("images", [])}
...
def split_dataset(src_dir, output_path, split_percentage: float = 0.8,
                format: str = "FUNSD", ann_filename: str = "instances_default.json"):
    """
    ...
    assert os.path.exists(ann_dir), "Source directory missing expected 'annotations' sub-directory"
    assert os.path.exists(img_dir), "Source directory missing expected 'images' sub-directory"
...
def default_split(args: object):
    ...
    dst_dir = args.dir_output if args.dir_output != "./split" else os.path.abspath(args.dir)
...
def get_split_parser(subparsers=None) -> argparse.ArgumentParser:
    ...
    if subparsers is not None:
        ...
        split_parser = argparse.ArgumentParser(prog="split_dataset", description="Split COCO dataset into train/test")
