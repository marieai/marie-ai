...
def extract_icr(image, label:str, boxp, icrp, debug_fragments: bool = False) -> (list, dict):
    ...
    if boxes is None or len(boxes) == 0:
        ...
        if debug_fragments:
            file_path = os.path.join(ensure_exists(f"{__tmp_path__}/icr/{checksum}/"), f"{checksum}.png")
...
def __decorate_funsd(
    ...
    for item in data["form"]:
        ...
        if debug_fragments:
            file_path = os.path.join(f"{__tmp_path__}/snippet", f"{filename}-snippet_{_id}.png")
...
def get_decorate_subparser(subparsers=None) -> argparse.ArgumentParser:
    ...
    if subparsers is not None:
        ...
        decorate_parser = argparse.ArgumentParser(prog="decorate", description="Fills converted FUNSD with ICR data")
