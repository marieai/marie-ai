...
vagrant_sync_images() {
    ...
    if [ ${#images_to_sync[@]} -eq 0 ]; then
        ...
        images_to_sync=(
            ...
            "marieai/marie-gateway:5.0.1-cpu"
            "marieai/marie:5.0.1-cuda"
            "marieai/marie:5.0.1-cpu"
        )
    fi
...
show_kubernetes_deployment_config() {
    ...
    local marie_image="${MARIE_IMAGE:-marieai/marie:5.0.1-cuda}"
    local gateway_image="${GATEWAY_IMAGE:-marieai/marie-gateway:5.0.1-cpu}"
