...
| **Gateway — Valkey / LLM queue** (`LLM_QUEUE_URL`, legacy `LLM_QUEUE_VALKEY_URL`, `LLM_QUEUE_*`) | `docker-compose.gateway.yml` selects canonical URL, then legacy URL, then `redis://localhost:6379/0` and forwards the selected value to both names | `charts/server/templates/deployment.yaml` selects `global.llmQueue.url`, an existing-secret reference, a configured `global.valkey.host`, or bundled `redis://<release>-valkey:6379/0` when both `global.valkey.enabled` and top-level `valkey.enabled` are true | **Covered** |
