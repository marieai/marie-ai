...
- Gateway: `marieai/marie-gateway:5.0.3-cpu`
- Server/executor: `marieai/marie:5.0.3-cuda`
...
- The gateway starts from `marieai/marie-gateway:5.0.3-cpu`.
- The gateway `/status` endpoint returns HTTP 200.
- The executor starts from `marieai/marie:5.0.3-cuda`.
...
```bash
docker image inspect marieai/marie-gateway:5.0.3-cpu
docker image inspect marieai/marie:5.0.3-cuda
docker run --rm --entrypoint python marieai/marie:5.0.3-cuda -c "import pkg_resources; print('pkg_resources ok')"
```
