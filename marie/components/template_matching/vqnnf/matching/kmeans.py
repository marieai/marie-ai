...
class KMeans:
    ...
    def __init__(
        ...
        self.estimated_memory = self.remaining_memory(device)
        # print("Estimated memory:", self.estimated_memory, "bytes")
        self.centroids = None
    ...
    def max_sim(self, a, b):
        ...
        if device.type == 'cpu':
            ...
            ratio = math.ceil(expected / self.estimated_memory)
    ...
    def fit_predict(self, X, centroids=None):
        ...
        if centroids is None:
            self.centroids = init_methods[self.init_method](
                X, self.n_clusters, self.minibatch
            )
        else:
            self.centroids = centroids
        num_points_in_clusters = torch.ones(
            self.n_clusters, device=device, dtype=X.dtype
        )
        closest = None
