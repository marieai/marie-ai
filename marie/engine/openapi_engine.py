...
class BatchProcessor:
    ...
    async def completion_non_streaming(self, messages, task_id, request_id):
        """
        ...
        try:
            ...
            completion_id = f"{request_id}-{task_id}"
        ...
        **kwargs,
        ...
        try:
            batch_outputs, task_results = asyncio.run(
                self.load_batched_request(messages_list, request_id)
            )
        except Exception as e:
...
class OpenAIEngine(EngineLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.model_string = models.data[0].id
    ...
    def batch_generate(
        ...
        **kwargs,
        ...
        self.batch_processor.batch_generate(messages_list)
        ...
        try:
            ...
            print(response)
            print(f'----------- : resonsed > {len(response.choices)}')
            for choice in response.choices:
                print(choice.message.content)

            ordered_outputs = [response]
        except Exception as e:
...
if __name__ == "__main__":
    ...
    print(engine)
    result = engine.generate(["Tell me an animal fact, for animal from Poland"] * 5)
    print(result)
