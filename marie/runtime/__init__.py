"""Public runtime API for the ``marie-ai`` distribution.

The top-level ``marie`` name is a PEP 420 namespace shared by independently
installed Marie distributions. Runtime users import the serving facade from
this module instead of relying on attributes attached to the namespace root.
"""
...
if _start_method and _start_method.lower() in {"fork", "spawn", "forkserver"}:
    from multiprocessing import get_start_method as _get_start_method
    from multiprocessing import set_start_method as _set_start_method

    _requested_start_method = _start_method.lower()
    _current_start_method = _get_start_method(allow_none=True)
    if _current_start_method is None:
        try:
            _set_start_method(_requested_start_method)
            _warnings.warn(
                f"multiprocessing start method is set to `{_requested_start_method}`"
            )
        except Exception as e:
            _warnings.warn(
                f"failed to set multiprocessing start_method to "
                f"`{_requested_start_method}`: {e!r}"
            )
    elif _current_start_method != _requested_start_method:
        _warnings.warn(
            f"multiprocessing start method is already `{_current_start_method}`; "
            f"cannot change it to `{_requested_start_method}`"
        )
    ...
    try:
        ...
        _warnings.warn('multiprocessing start method is set to `fork`')
    except Exception as e:
