...
from marie.core.vector_stores.types import VectorStoreQuery
...
class SimplePropertyGraphStore(PropertyGraphStore):
    ...
    def get(
        ...
        # Filter by node_ids
        if ids:
            nodes = [n for n in nodes if n.task_id in ids]
    ...
    def get_triplets(
        ...
        if entity_names:
            triplets = [
                t
                for t in triplets
                if t[0].task_id in entity_names or t[2].task_id in entity_names
            ]

        if relation_names:
            triplets = [t for t in triplets if t[1].task_id in relation_names]
        ...
        if ids:
            triplets = [
                t for t in triplets if any(t[0].task_id == i or t[2].task_id == i for i in ids)
            ]
    ...
    def save_networkx_graph(self, name: str = "kg.html") -> None:
        ...
        for node in self.graph.nodes.values():
            G.add_node(node.task_id, label=node.task_id)
    ...
    def show_jupyter_graph(self) -> None:
        ...
        edges = []
        for node in self.graph.nodes.values():
            node_dict = {"id": node.task_id, "properties": {"label": node.task_id}}
