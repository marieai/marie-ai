...
class StepHandle(
    NamedTuple("_StepHandle", [("node_handle", NodeHandle), ("key", str)])
):
    ...
    def __new__(cls, node_handle: NodeHandle, key: Optional[str] = None):
        return super(StepHandle, cls).__new__(
            ...
            key=cast(
                str, check.opt_str_param(key, "key", default=node_handle.to_string())
            ),
        )
...
class UnresolvedStepHandle(
    NamedTuple("_UnresolvedStepHandle", [("node_handle", NodeHandle)])
):
...
class ResolvedFromDynamicStepHandle(
    ...
    def __new__(
        cls, node_handle: NodeHandle, mapping_key: str, key: Optional[str] = None
    ):
