    ...
    if opt.saved_model != '':
        ...
        if opt.FT:
            model.load_state_dict(
                torch.load(opt.saved_model, weights_only=True), strict=False
            )
        else:
            model.load_state_dict(torch.load(opt.saved_model, weights_only=True))
    print("Model:")
    print(model)
...
if __name__ == '__main__':
    ...
    opt.character = r'0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!"#$%&\'()*+,-./:;<=>?@[\]^_`{|}~'
    opt.character = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!"#$%&\'()*+,-./:;<=>?@[\]^_`{|}~'
