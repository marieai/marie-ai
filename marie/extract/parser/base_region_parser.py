...
from marie.extract.parser.base import normalize_row_values
...
class BaseRegionParser(ABC):
    """
    Abstract base class for region parsers that convert structured content
    (markdown, JSON, etc.) into StructuredRegion objects.

    Provides common functionality for:
    - Configuration management and validation
    - Section role and title normalization
    - Table and KV section building
    - Multi-page region construction
    """
    ...
    def __init__(
        ...
        # Indicates whether the parser was constructed via from_config(strict)
        self._has_sections_cfg: bool = getattr(self, "_has_sections_cfg", False)
    ...
    def _split_sets_by_parse(
        ...
        for item in sections_cfg or []:
            ...
            if not title:
                continue
            key = BaseRegionParser._norm_title_key(title)
    ...
    @classmethod
    def from_config(cls, cfg: Dict[str, object], **kwargs) -> "BaseRegionParser":
        """
        ...
        # Mark that we have an explicit sections config; parsing must be restricted to it
        inst._has_sections_cfg = True
        return inst

    def summary(self) -> Dict[str, object]:
        """
        Return a concise snapshot of the parser's configuration.
        """
        return {
    ...
    def build_single_page_region(
        ...
        *,
        ...
        for title_uc, content in sections_seq:
            role = self._role_for(title_uc)
            role_hint = self._role_hint_for(title_uc)

            # KV section (optional)
            kv_sec = self._build_kv_section(title_uc, role, content)
            ...
            headers, rows = self._parse_table_content(content)
            if headers:
    ...
    def build_multi_page_region(
        ...
        *,
        ...
        """
        Parse data into a two-page StructuredRegion (generic multi-page demo).
        """
        sections_seq = self._parse_and_validate_sections(data)
    ...
    @abstractmethod
    def _parse_and_validate_sections(self, data: Any) -> List[Tuple[str, Any]]:
        """
        Parse input data into sections and validate.
        Returns list of (TITLE_UC, content) pairs.

        Must be implemented by subclasses to handle their specific format.
        """
        pass

    @abstractmethod
    def _parse_table_content(self, content: Any) -> Tuple[List[str], List[List[str]]]:
        """
        Parse section content into table format (headers, rows).

        Must be implemented by subclasses to handle their specific content format.
        """
        pass

    @abstractmethod
    def _build_kv_section(
        self, title_uc: str, role: SectionRole, content: Any
    ) -> Optional["Section"]:
        """
        Build a KVList Section from content if appropriate.
    ...
    def _validate_sections(self, sections: List[Tuple[str, Any]]) -> None:
        """
        - No empty section titles
        - No duplicate section titles
        """
        # Empty title check
        empty_titles = [t for t, _ in sections if not (t or "").strip()]
        if empty_titles:
            raise ValueError("Empty section titles are not allowed")
    ...
    def _build_table_section(
        ...
        *,
        ...
        """
        # Enforce filtering: if a strict config is present, only allow explicitly configured table sections.
        key = self._norm_title_key(title_uc)
        has_cfg = getattr(self, "_has_sections_cfg", False)
        if has_cfg:
            if key not in self.table_section_titles:
                return None
        else:
