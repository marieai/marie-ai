...
def coerce_region_processor_fn(
    ...
    """Normalize to (context, parent_section, region_parser_config, regions_config, template_fields) -> List[Dict]."""
    if inspect.isclass(obj):
        if not hasattr(obj, "process"):
            raise TypeError(
                f"{obj.__name__} must define .process(self, context, parent_section, region_parser_config, regions_config)."
            )
        if not has_zero_arg_init(obj):
            raise TypeError(
                f"{obj.__name__}.__init__ must be zero-arg; register an instance or a function instead."
            )
        return lambda context, parent_section, region_parser_config, regions_config: obj().process(
            context, parent_section, region_parser_config, regions_config
    ...
    if callable(obj):
        ...
        if count_required_params(sig) != 4:
            raise TypeError(
                ...
                "(context, parent_section, region_parser_config, regions_config, template_fields)."
            )
