...
class ConfigTypeKind(PythonEnum):
    ...
    def is_shape(kind: "ConfigTypeKind") -> bool:
        ...
        return (
            ...
            or kind == ConfigTypeKind.PERMISSIVE_SHAPE
        )
...
class ConfigType:
    ...
    def __init__(
        ...
        self._description: Optional[str] = check.opt_str_param(
    ...
    def from_builtin_enum(builtin_enum: typing.Any) -> "ConfigType":
        check.invariant(
            BuiltinEnum.contains(builtin_enum), "param must be member of BuiltinEnum"
        )
    ...
    def get_schema_snapshot(self) -> "ConfigSchemaSnapshot":
        ...
        return ConfigSchemaSnapshot(
            {ct.key: ct.get_snapshot() for ct in self.type_iterator()}
        )
...
class ConfigScalar(ConfigType):
    def __init__(
        ...
        **kwargs: object,
        ...
        self.scalar_kind = check.inst_param(
            scalar_kind, "scalar_kind", ConfigScalarKind
        )
...
class String(BuiltinConfigScalar):
    def __init__(self):
        super(String, self).__init__(
            scalar_kind=ConfigScalarKind.STRING, description=""
        )
...
class Enum(ConfigType):
    ...
    **Examples:**
    ...
    def __init__(self, name: str, enum_values: Sequence[EnumValue]):
        ...
        self.enum_values = check.sequence_param(
            enum_values, "enum_values", of_type=EnumValue
        )
    ...
    **Examples:**
    ...
    def __init__(
        ...
        check.param_invariant(
            self.scalar_type.kind == ConfigTypeKind.SCALAR, "scalar_type"
        )
        check.param_invariant(
            self.non_scalar_type.kind
            in {
                ConfigTypeKind.STRICT_SHAPE,
                ConfigTypeKind.SELECTOR,
