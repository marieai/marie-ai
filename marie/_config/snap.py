...
class ConfigTypeSnap(
    NamedTuple(
        ...
        [
            ...
            ),  # only valid for closed generics
            (
                "enum_values",
                Optional[Sequence["ConfigEnumValueSnap"]],
            ),  # only valid for enums
            (
                "fields",
                Optional[Sequence["ConfigFieldSnap"]],
            ),  # only valid for dicts and selectors
            ("scalar_kind", Optional[ConfigScalarKind]),  # only valid for scalars
            (
                "field_aliases",
                Optional[Mapping[str, str]],
            ),  # only valid for strict shapes
        ],
    )
):
    # serdes log
    ...
    def __new__(
        ...
        return super(ConfigTypeSnap, cls).__new__(
            ...
            if enum_values is None
                ...
                enum_values, "enum_values", of_type=ConfigEnumValueSnap
            ),
            fields=None
            if fields is None
            else sorted(
            ...
            scalar_kind=check.opt_inst_param(
...
def minimal_config_for_type_snap(
    ...
    if ConfigTypeKind.has_fields(config_type_snap.kind):
        ...
        return (
            config_type_snap.enum_values[0].value
            if config_type_snap.enum_values
            else "<unknown>"
        )
