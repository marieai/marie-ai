...
class EvaluationStack(
    ...
    def for_field(self, field_name: str) -> "EvaluationStack":
        return EvaluationStack(
            entries=[*self.entries, EvaluationStackPathEntry(field_name)]
        )

    def for_array_index(self, list_index: int) -> "EvaluationStack":
        return EvaluationStack(
            entries=[*self.entries, EvaluationStackListItemEntry(list_index)]
        )

    def for_map_key(self, map_key: object) -> "EvaluationStack":
        return EvaluationStack(
            entries=[*self.entries, EvaluationStackMapKeyEntry(map_key)]
        )

    def for_map_value(self, map_key: object) -> "EvaluationStack":
        return EvaluationStack(
            entries=[*self.entries, EvaluationStackMapValueEntry(map_key)]
        )
...
class EvaluationStackListItemEntry(
    NamedTuple("_EvaluationStackListItemEntry", [("list_index", int)]),
