...
from marie.utils.indenting_printer import IndentingPrinter
...
def _print_type_from_config_type(config_type, print_fn=print, with_lines=True):
    ...
    return _print_type(
        config_type.get_schema_snapshot(), config_type.key, print_fn, with_lines
    )
...
def _do_print(config_schema_snapshot, config_type_key, printer, with_lines=True):
    line_break_fn = (
        printer.line if with_lines else lambda string: printer.append(string + " ")
    )
...
def print_config_type_key_to_string(
    config_schema_snapshot, config_type_key, with_lines=True
):
    prints = []
...
def print_config_type_to_string(config_type, with_lines=True):
    ...
    _print_type_from_config_type(
        resolve_to_config_type(config_type), _push, with_lines=with_lines
    )
