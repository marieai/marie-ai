...
from marie.utils import ensure_single_item
...
def is_config_scalar_valid(
    config_type_snap: ConfigTypeSnap, config_value: object
) -> bool:
    check.inst_param(config_type_snap, "config_type_snap", ConfigTypeSnap)
    check.param_invariant(
        config_type_snap.kind == ConfigTypeKind.SCALAR, "config_type_snap"
    )
...
def _validate_scalar_union_config(
    ...
    check.inst_param(context, "context", ValidationContext)
    check.param_invariant(
        context.config_type_snap.kind == ConfigTypeKind.SCALAR_UNION, "context"
    )
    check.not_none_param(config_value, "config_value")

    if isinstance(config_value, (dict, list)):
        return _validate_config(
            context.for_new_config_type_key(
                context.config_type_snap.non_scalar_type_key
            ),
            cast(T, config_value),
        )
    else:
...
def validate_selector_config(
    ...
    check.inst_param(context, "context", ValidationContext)
    check.param_invariant(
        context.config_type_snap.kind == ConfigTypeKind.SELECTOR, "selector_type"
    )
    check.not_none_param(config_value, "config_value")
    ...
    if not isinstance(config_value, dict):
        return EvaluateValueResult.for_error(
            create_selector_type_error(context, config_value)
        )
...
def _validate_shape_config(
    ...
    _append_if_error(
        ...
        _compute_missing_fields_error(
            context, field_snaps, incoming_field_names, field_aliases
        ),
    )
    ...
    for field_snap in field_snaps:
        ...
        if (
            ...
            and aliased_name in config_value
            and name in config_value
        ):
            field_errors.append(
                create_field_substitution_collision_error(
                    context.for_field_snap(field_snap),
            ...
            field_evr = _validate_config(
                context.for_field_snap(field_snap), config_value[name]
            )
...
def validate_permissive_shape_config(
    ...
    return _validate_shape_config(
        context, config_value, check_for_extra_incoming_fields=False
    )
...
def validate_shape_config(
    ...
    return _validate_shape_config(
        context, config_value, check_for_extra_incoming_fields=True
    )


def _append_if_error(
    errors: List[EvaluationError], maybe_error: Optional[EvaluationError]
) -> None:
    if maybe_error:
...
def _check_for_extra_incoming_fields(
    context: ValidationContext,
    defined_field_names: Set[str],
    incoming_field_names: Set[str],
...
def validate_enum_config(
    ...
    if not isinstance(config_value, str):
        return EvaluateValueResult.for_error(
            create_enum_type_mismatch_error(context, config_value)
        )

    if not context.config_type_snap.has_enum_value(config_value):
        return EvaluateValueResult.for_error(
            create_enum_value_missing_error(context, config_value)
        )
