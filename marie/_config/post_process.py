...
def post_process_config(
    config_type: ConfigType, config_value: Any
    ...
    if kind == ConfigTypeKind.SCALAR:
        ...
        if config_value is None:
            ...
            return _recursively_process_config(
                context.for_nullable_inner_type(), config_value
            )
...
def _recurse_in_to_selector(
    ...
    if config_value:
        ...
        incoming_field_value = (
            field_def.default_value if field_def.default_provided else None
        )
...
def _recurse_in_to_shape(
    ...
    check.invariant(
        ConfigTypeKind.is_shape(context.config_type.kind), "Unexpected non shape type"
    )
    ...
    for expected_field, field_def in fields.items():
        if expected_field in incoming_fields:
            ...
            expected_field in field_aliases
            and field_aliases[expected_field] in incoming_fields
        ):
            processed_fields[expected_field] = _recursively_process_config(
                context.for_field(field_def, expected_field),
...
def _recurse_in_to_array(
    context: TraversalContext, config_value: Any
) -> EvaluateValueResult[Any]:
    check.invariant(
        context.config_type.kind == ConfigTypeKind.ARRAY, "Unexpected non array type"
    )
...
def _recurse_in_to_map(
    ...
    return EvaluateValueResult.for_value(
        {key: result.value for key, result in results.items()}
    )
