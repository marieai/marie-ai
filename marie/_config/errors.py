...
class FieldNotDefinedErrorData(
    NamedTuple("_FieldNotDefinedErrorData", [("field_name", str)])
):
...
class MissingFieldErrorData(
    NamedTuple(
        "_MissingFieldErrorData", [("field_name", str), ("field_snap", ConfigFieldSnap)]
    )
):
...
class RuntimeMismatchErrorData(
    NamedTuple(
        "_RuntimeMismatchErrorData",
        [("config_type_snap", ConfigTypeSnap), ("value_rep", str)],
    )
):
...
class SelectorTypeErrorData(
    ...
    def __new__(cls, config_type_snap, incoming_fields):
        ...
        check.param_invariant(
            config_type_snap.kind == ConfigTypeKind.SELECTOR, "config_type"
        )
...
def create_dict_type_mismatch_error(
    context: ContextData, config_value: object
) -> EvaluationError:
...
def create_field_substitution_collision_error(
    ...
    return EvaluationError(
        ...
        error_data=FieldAliasCollisionErrorData(
...
def create_enum_type_mismatch_error(
    context: ContextData, config_value: object
) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
    ...
    return EvaluationError(
        ...
        error_data=RuntimeMismatchErrorData(
            context.config_type_snap, repr(config_value)
        ),
    )


def create_enum_value_missing_error(
    context: ContextData, config_value: object
) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
...
def check_config_type_in_context_has_fields(
    context: ContextData, param_name: str
) -> None:
    check.param_invariant(
        ConfigTypeKind.has_fields(context.config_type_snap.kind), param_name
    )


def create_field_not_defined_error(
    context: ContextData, received_field: str
) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
    check_config_type_in_context_has_fields(context, "context")
...
def create_array_error(context: ContextData, config_value: object) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
    check.param_invariant(
        context.config_type_snap.kind == ConfigTypeKind.ARRAY, "config_type"
    )
...
def create_map_error(context: ContextData, config_value: object) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
    check.param_invariant(
        context.config_type_snap.kind == ConfigTypeKind.MAP, "config_type"
    )
...
def create_missing_required_field_error(
    ...
    return EvaluationError(
        ...
        error_data=MissingFieldErrorData(
            ...
            field_snap=context.config_type_snap.get_field(expected_field),
        ),
    )
            ...
            " entries: {minimal_config}".format(
                ...
                minimal_config={
                    field_snap.name: minimal_config_for_type_snap(
                        context.config_schema_snapshot,
                        context.config_schema_snapshot.get_config_snap(
                            field_snap.type_key
                        ),
                    )
...
def create_selector_multiple_fields_no_field_selected_error(
    ...
    return EvaluationError(
        ...
        message=(
            ...
            defined_fields=defined_fields, path_msg=get_friendly_path_msg(context.stack)
        ),
        error_data=SelectorTypeErrorData(
...
def create_selector_type_error(
    context: ContextData, config_value: object
) -> EvaluationError:
    check.inst_param(context, "context", ContextData)
