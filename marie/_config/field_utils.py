...
from marie._core.errors import DagsterInvalidConfigDefinitionError

from .config_type import Array, ConfigType, ConfigTypeKind

if TYPE_CHECKING:
    from marie._config import Field
...
def _define_shape_key_hash(fields, description, field_aliases):
    return "Shape." + compute_fields_hash(
        fields, description, field_aliases=field_aliases
    )
...
class Shape(_ConfigHasFields):
    ...
    def __new__(
        ...
        return _memoize_inst_in_field_cache(
            cls,
            Shape,
            _define_shape_key_hash(
                expand_fields_dict(fields), description, field_aliases
            ),
        )
...
def expand_list(
    original_root: object, the_list: Sequence[object], stack: List[str]
) -> Array:
    if len(the_list) != 1:
...
def expand_map(
    original_root: object, the_dict: Mapping[object, object], stack: List[str]
) -> Map:
    if len(the_dict) != 1:
        raise DagsterInvalidConfigDefinitionError(
...
def _convert_potential_type(original_root: object, potential_type, stack: List[str]):
    ...
    if isinstance(potential_type, Mapping):
        ...
        if len(potential_type) == 1:
            ...
            if not isinstance(key, str) and _convert_potential_type(
                original_root, key, stack
            ):
...
def config_dictionary_from_values(
    ...
    # from marie._config.pythonic_config import _config_value_to_dict_representation

    # return check.is_dict(_config_value_to_dict_representation(None, values))
    return check.is_dict(values)
