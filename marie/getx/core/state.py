...
from marie.getx.core import serde  # Assuming you have a serde module
from marie.getx.core.typing import DictBasedTypingSystem, TypingSystem
...
def register_field_serde(field_name: str, serializer: Callable, deserializer: Callable):
    ...
    if not any(
        param.kind == param.VAR_KEYWORD for param in serializer_sig.parameters.values()
    ):
    ...
    if not any(
        ...
        for param in deserializer_sig.parameters.values()
    ):
        raise ValueError(
            f"Deserializer for [{field_name}] must have **kwargs argument."
        )
...
class StateDelta(abc.ABC):
    ...
    def serialize(self) -> dict:
        ...
        if not dataclasses.is_dataclass(self):
            raise TypeError(
                "serialize method is only supported for dataclass instances"
            )
    ...
    def deserialize(cls, json_dict: dict) -> "StateDelta":
        ...
        return cls(
            **json_dict
        )  # Assumes all fields in the dataclass match keys in json_dict
...
class AppendFields(StateDelta):
    ...
    def validate(self, input_state: Dict[str, Any]):
        ...
        for write_key in self.writes():
            if write_key in input_state and not hasattr(
                input_state[write_key], "append"
            ):
...
class State(Mapping, Generic[StateType]):
    """An immutable state object. This is the only way to interact with state in Marie."""
    ...
    def wipe(
        self, delete: Optional[list[str]] = None, keep: Optional[list[str]] = None
    ):
        ...
        if delete is not None:
            ...
            fields_to_delete = [key for key in self._state if key not in (keep or [])]
...
for serde_plugin in ["pydantic", "pickle"]:  # Marie-specific plugins
    try:
        importlib.import_module(f"marie.integrations.serde.{serde_plugin}")
    except ImportError:
