...
class MarieServerGateway(CompositeServer):
    ...
    def __init__(self, **kwargs):
        ...
        def _extend_rest_function(app: 'FastAPI'):
            ...
            @app.get(
                path="/api/connectors",
                summary="List all registered connectors with metadata",
                tags=["Connectors"],
            )
            async def list_connectors():
                from marie.connectors.registry import ConnectorRegistry

                connectors = ConnectorRegistry.list_connectors_with_metadata()
                return {"connectors": connectors, "total": len(connectors)}

            @app.get(
                path="/api/connectors/{connector_id:path}",
                summary="Get a specific connector by ID",
                tags=["Connectors"],
            )
            async def get_connector(connector_id: str, response: Response):
                from fastapi import status as http_status

                from marie.connectors.registry import ConnectorRegistry

                manifest = ConnectorRegistry.get(connector_id)
                if manifest is None:
            ...
            @app.post(
                path="/api/connectors/deploy",
                summary="Deploy a connector from a file bundle",
                tags=["Connectors"],
                status_code=201,
            )
            async def deploy_connector(request: Request, response: Response):
                """Deploy a connector by writing a bundle of files and registering it.

                Request body:
                    connector_id: Connector ID (e.g. "connector.bitly")
                    files: Dict of relative path -> file content
                    source_type: "deployed" (default) or "thirdparty"
                ...
                if not files or not isinstance(files, dict):
                    ...
                    return {
                        "success": False,
                        "error": "Missing or invalid 'files' dict",
                    }

                source_type = body.get("source_type", "deployed")
                overwrite = body.get("overwrite", False)

                # Determine connector base directory from the connectors package
                try:
                    import marie.connectors as _cpkg

                    connectors_base = str(_Path(_cpkg.__file__).parent)
                except Exception:
                    response.status_code = http_status.HTTP_500_INTERNAL_SERVER_ERROR
                    return {
                        "success": False,
                        "error": "Cannot resolve connectors base directory",
                    }

                # Extract node name from connector.yaml id field
                import yaml

                yaml_content = files.get("connector.yaml", "")
                if not yaml_content:
                    response.status_code = http_status.HTTP_400_BAD_REQUEST
                    return {
                        "success": False,
                        "error": "Bundle must include connector.yaml",
                    }
                ...
                try:
                    ...
                    return {
                        "success": False,
                        "error": f"Invalid connector.yaml: {e}",
                    }

                # Derive directory name from connector ID (e.g. "connector.bitly" -> "bitly")
                dir_name = (
                    connector_id.rsplit(".", 1)[-1]
                    if "." in connector_id
                    else connector_id
                )
                target_dir = str(_Path(connectors_base) / dir_name)

                try:
                    manifest = ConnectorRegistry.register_from_bundle(
                        target_dir=target_dir,
                        files=files,
                        source_type=source_type,
                        overwrite=overwrite,
                    )
                    # Return full metadata for the newly deployed connector
                    metadata = ConnectorRegistry.list_connectors_with_metadata()
                    connector_meta = next(
                        (c for c in metadata if c["id"] == manifest.id),
                        None,
                    )
                    return {
                        "success": True,
                        "message": f"Connector '{manifest.id}' deployed successfully",
                    ...
                    self.logger.error(f"Failed to deploy connector: {e}")
                    response.status_code = http_status.HTTP_500_INTERNAL_SERVER_ERROR
                    return {"success": False, "error": str(e)}

            @app.delete(
                path="/api/connectors/{connector_id:path}",
                summary="Unregister and optionally remove a connector",
                tags=["Connectors"],
            )
            async def delete_connector(
                ...
                """Unregister a deployed/thirdparty connector.

                Query params:
                    remove_files: If true (default), also delete the connector directory.
                """
                import shutil
                from pathlib import Path as _Path
                ...
                if remove_files and manifest.package_name:
                    try:
                        pkg = importlib.import_module(manifest.package_name)
                        if pkg.__file__:
                            pkg_dir = str(_Path(pkg.__file__).parent)
                            shutil.rmtree(pkg_dir, ignore_errors=True)
                            self.logger.info(f"Removed connector directory: {pkg_dir}")
                    except ImportError:
                        self.logger.warning(
                            f"Could not resolve package dir for {manifest.package_name}"
                        )

                return {
                    "success": True,
                    "message": f"Connector '{connector_id}' unregistered",
                }

            @app.post(
                path="/api/connectors/{connector_id:path}/reload",
                summary="Hot-reload a single connector",
                tags=["Connectors"],
            )
            async def reload_connector(connector_id: str, response: Response):
                """Re-read connector.yaml and re-import backend modules."""
                from pathlib import Path as _Path
                ...
                if not manifest.package_name:
                    ...
                    return {
                        "success": False,
                        "error": "Connector has no package_name — cannot reload",
                    }

                try:
                    pkg = importlib.import_module(manifest.package_name)
                    if not pkg.__file__:
                        raise ImportError("Package has no __file__")
                    yaml_path = str(_Path(pkg.__file__).parent / "connector.yaml")

                    old_source_type = manifest.source_type
                    # Remove old entry so register() accepts the new one
                    ConnectorRegistry._connectors.pop(connector_id, None)
                    ConnectorRegistry.register_from_yaml(
                        yaml_path,
                    ...
                    return {
                        "success": True,
                        "message": f"Connector '{connector_id}' reloaded",
                    }
                except Exception as e:
                    self.logger.error(
                        f"Failed to reload connector '{connector_id}': {e}"
                    )
                    response.status_code = http_status.HTTP_500_INTERNAL_SERVER_ERROR
                    return {"success": False, "error": str(e)}
            ...
            async def get_registry_info():
                ...
                try:
                    ...
                    connector_info = ConnectorRegistry.get_registry_info()
