    ...
    class _ReqReplyNode:
        ...
        def _handle_internalnetworkerror(self, err):
            ...
            if err_code == grpc.StatusCode.UNAVAILABLE:
                err._details = (
                    err.details()
                    + f' |Gateway: Communication error with deployment {self.name} at'
                    f' address(es) {err.dest_addr}. Head or worker(s) may be down.'
                )
                raise err
            elif err_code == grpc.StatusCode.DEADLINE_EXCEEDED:
                err._details = (
                    err.details()
                    + f'|Gateway: Connection with deployment {self.name} at address(es)'
                    f' {err.dest_addr} could be established, but timed out. You can'
                    ' increase the allowed time by setting `timeout_send` in your'
                    ' Flow YAML `with` block or Flow `__init__()` method.'
                )
                raise err
            else:
                raise
    ...
    class _EndGatewayNode(_ReqReplyNode):
        ...
        def get_endpoints(self, *args, **kwargs) -> asyncio.Task:
            async def task_wrapper():
                from marie.serve.networking import default_endpoints_proto
