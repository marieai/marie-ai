...
def opt_class_param(
    ...
    if obj is not None and not isinstance(obj, type):
        raise _param_class_mismatch_exception(
            obj, param_name, superclass, True, additional_message
        )
...
def inst(
    obj: T, ttype: TypeOrTupleOfTypes, additional_message: Optional[str] = None
) -> T:
    if not isinstance(obj, ttype):
...
def iterator_param(
    ...
    if not isinstance(obj, Iterator):
        raise _param_type_mismatch_exception(
            obj, Iterator, param_name, additional_message
        )
    return obj
...
def two_dim_list_param(
    ...
    obj = list_param(
        obj, param_name, of_type=list, additional_message=additional_message
    )
    if not obj:
...
def mapping_param(
    ...
    return _check_mapping_entries(
        obj, key_type, value_type, mapping_type=collections.abc.Mapping
    )
...
def numeric_param(
    ...
    if not isinstance(obj, (int, float)):
        raise _param_type_mismatch_exception(
            obj, (int, float), param_name, additional_message
        )
    return obj
...
def path_param(
    ...
    if not isinstance(obj, (str, PathLike)):
        raise _param_type_mismatch_exception(
            obj, (str, PathLike), param_name, additional_message
        )
    return fspath(obj)
...
def set_param(
    ...
    if not isinstance(obj, (frozenset, set)):
        raise _param_type_mismatch_exception(
            obj, (frozenset, set), param_name, additional_message
        )
        ...
        for i, obj in enumerate(obj_tuple):
            ...
            if not isinstance(obj, of_shape_i):
                if isinstance(obj, type):
                    additional_message = " Did you pass a class where you were expecting an instance of the class?"
                else:
                    additional_message = ""
                raise CheckError(
...
def _param_invariant_exception(
    param_name: str, desc: Optional[str] = None
) -> ParameterCheckError:
    return ParameterCheckError(
