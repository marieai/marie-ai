...
def get_ocr_line_bbox(bbox, frame, executor):
...
def aggregate_results(
    ...
    for i, (ocr, ann, frame) in enumerate(zip(ocr_results, annotation_results, frames)):
        logger.info("Processing page # %s", i)
        ...
        # frame.show()
        viz_img.save(f"/tmp/tensors/extract_{file_hash}.png")
        ...
        for agg_result in aggregated_kv:
            ...
            w1, c1 = get_ocr_line_bbox(question["bbox"], frame, executor)
            w2, c2 = get_ocr_line_bbox(answer["bbox"], frame, executor)
...
def main_image(
    src_image: str, model, device, executor: Optional[TextExtractionExecutor] = None
):
    if not os.path.exists(src_image):
    ...
    if not os.path.exists(ocr_json_path) and executor is None:
    ...
    if not os.path.exists(ocr_json_path):
        results, frames = obtain_ocr(src_image, executor)
...
def ocr_dir(src_dir, output_dir, filename_filter="*.png"):
    print(f"OCR inpout directory : {src_dir}")
    ...
    for idx, _path in enumerate(glob.glob(os.path.join(src_dir, filename_filter))):
        try:
            ...
            if os.path.exists(json_path):
                print(f"OCR output exists : {json_path}, {_path}")
                continue

            results, frames = obtain_ocr(_path, executor)
            store_json_object(results, json_path)
            visualize_icr(frames, results, filename)
        except BaseException as e:
            logger.error(f"Unable to ocr : {_path}")
...
if __name__ == "__main__":
    ...
    if False:
        ...
        print(f"marie_home = {home}")
        executor = None  # TextExtractionExecutor()
        executor = TextExtractionExecutor()

        for idx, img_path in enumerate(
            glob.glob(os.path.join(image_dir, "PID_1898_9200_0_156692423.png"))
        ):
            try:
                main_image(img_path, model, device, executor)
                ner_results = aggregate_results(img_path, executor)
                break
    ...
    if False:
        marie_home = os.path.join(str(Path.home()), ".marie")
        ...
        for idx, _path in enumerate(glob.glob(os.path.join(marie_home, "*"))):
            ...
            image_path = f"/data/dataset/private/corr-indexer/validation/{filename}"
            image_path = (
                f"/home/greg/dataset/assets-private/corr-indexer/validation/{filename}"
            )
