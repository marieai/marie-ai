...
if TYPE_CHECKING:
    from marie._core.workspace.context import IWorkspace
...
class SubmitRunContext(NamedTuple):
    ...
    def get_request_header(self, key: str) -> Optional[str]:
        from marie._core.workspace.context import WorkspaceRequestContext

        # if there is a source
        if (
            isinstance(self.workspace, WorkspaceRequestContext)
            and self.workspace.source
        ):
