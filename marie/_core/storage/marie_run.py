...
from marie._core.origin import JobPythonOrigin
...
from marie._core.utils import make_new_run_id
...
class DagsterRun(
    ...
    def is_failure_or_canceled(self):
        return (
            self.status == DagsterRunStatus.FAILURE
            or self.status == DagsterRunStatus.CANCELED
        )
...
class RunsFilter(
    ...
    def __new__(
        ...
        check.invariant(
            run_ids != [], "When filtering on run ids, a non-empty list must be used."
        )
        ...
        return super(RunsFilter, cls).__new__(
            ...
            statuses=check.opt_sequence_param(
                statuses, "statuses", of_type=DagsterRunStatus
            ),
            tags=check.opt_mapping_param(tags, "tags", key_type=str),
            snapshot_id=check.opt_str_param(snapshot_id, "snapshot_id"),
            updated_after=check.opt_inst_param(
                updated_after, "updated_after", datetime
            ),
            updated_before=check.opt_inst_param(
                updated_before, "updated_before", datetime
            ),
            created_after=check.opt_inst_param(
                created_after, "created_after", datetime
            ),
            created_before=check.opt_inst_param(
                created_before, "created_before", datetime
            ),
        )
...
class RunRecord(
    ...
    def __new__(
        ...
        return super(RunRecord, cls).__new__(
            ...
            create_timestamp=check.inst_param(
                create_timestamp, "create_timestamp", datetime
            ),
            update_timestamp=check.inst_param(
                update_timestamp, "update_timestamp", datetime
            ),
