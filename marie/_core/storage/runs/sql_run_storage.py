...
from marie._core.events import (
    EVENT_TYPE_TO_PIPELINE_RUN_STATUS,
    DagsterEvent,
    DagsterEventType,
)
from marie._core.execution.backfill import BulkActionStatus, PartitionBackfill
from marie._core.host_representation.origin import ExternalJobOrigin
from marie._core.snap import (
    ExecutionPlanSnapshot,
    JobSnapshot,
    create_execution_plan_snapshot_id,
    create_job_snapshot_id,
)
from marie._core.storage.sql import SqlAlchemyQuery, SqlAlchemyRow
from marie._core.storage.tags import (
    PARTITION_NAME_TAG,
    PARTITION_SET_TAG,
...
class SqlRunStorage(RunStorage):
    ...
    def add_run(self, dagster_run: DagsterRun) -> DagsterRun:
        ...
        if dagster_run.job_snapshot_id and not self.has_job_snapshot(
    ...
    def handle_run_event(self, run_id: str, event: DagsterEvent) -> None:
        ...
        if (
            run_stats_cols_in_index
            and event.event_type == DagsterEventType.PIPELINE_START
        ):
    ...
    def _add_cursor_limit_to_query(
        ...
        if cursor:
            cursor_query = db.select([RunsTable.c.id]).where(
    ...
    def _add_filters_to_query(
        self, query: SqlAlchemyQuery, filters: RunsFilter
    ) -> SqlAlchemyQuery:
    ...
    def _runs_query(
        ...
        filters = check.opt_inst_param(
            filters, "filters", RunsFilter, default=RunsFilter()
        )
        ...
        if bucket_by:
            ...
            return self._bucketed_runs_query(
                bucket_by, filters, columns, order_by, ascending
            )
        ...
        return self._add_cursor_limit_to_query(
            base_query, cursor, limit, order_by, ascending
        )
    ...
    def _bucket_rank_column(
        ...
        check.invariant(
            self.supports_bucket_queries,
            "Bucket queries are not supported by this storage layer",
        )
        sorting_column = getattr(RunsTable.c, order_by) if order_by else RunsTable.c.id
        direction = db.asc if ascending else db.desc
        bucket_column = (
        ...
        with self.connect() as conn:
            ...
            for tag in existing_tags:
                conn.execute(
                    ...
                    .where(
                        db.and_(
                            RunTagsTable.c.run_id == run_id, RunTagsTable.c.key == tag
                        )
                    )
    ...
    def get_run_group(self, run_id: str) -> Tuple[str, Sequence[DagsterRun]]:
        ...
        # find root_run
        root_run_id = (
            dagster_run.root_run_id if dagster_run.root_run_id else dagster_run.run_id
        )
        ...
        root_to_run = (
            ...
            .where(
                db.and_(
                    RunTagsTable.c.key == ROOT_RUN_ID_TAG,
                    RunTagsTable.c.value == root_run_id,
                )
            )
            .alias("root_to_run")
        )
    ...
    def get_run_groups(
        ...
        runs_and_root_runs_with_descendant_counts = (
            ...
            .select_from(
                RunsTable.join(
                    runs_and_root_runs,
                    RunsTable.c.run_id == runs_and_root_runs.c.run_id,
                ).join(
                    all_descendant_runs,
        ...
        for row in res:
            ...
            if root_run_id is not None:
                ...
                root_run_id_to_count[dagster_run.run_id] = (
                    cast(int, row["child_counts"]) + 1
                )
    ...
    def _add_snapshot(
        self, snapshot_id: str, snapshot_obj, snapshot_type: SnapshotType
    ) -> str:
        ...
        with self.connect() as conn:
            snapshot_insert = SnapshotsTable.insert().values(
                ...
                snapshot_body=zlib.compress(
                    serialize_value(snapshot_obj).encode("utf-8")
                ),
    ...
    def get_run_storage_id(self) -> str:
        ...
        if not row:
            ...
            with self.connect() as conn:
                conn.execute(
                    InstanceInfo.insert().values(run_storage_id=run_storage_id)
                )
    ...
    def get_run_partition_data(
        ...
        if self.has_built_index(RUN_PARTITIONS) and self.has_run_stats_index_cols():
            ...
            for row in rows:
                if (
                    not row["partition"]
                    or row["partition"] in _partition_data_by_partition
                ):
                    continue
        ...
        for migration_name, migration_fn in migrations.items():
            if self.has_built_index(migration_name):
                if not force_rebuild_all:
                    if print_fn:
                        print_fn(
                            f"Skipping already applied data migration: {migration_name}"
                        )
                    continue
            if print_fn:
    ...
    def migrate(
        self, print_fn: Optional[PrintFn] = None, force_rebuild_all: bool = False
    ) -> None:
        self._execute_data_migrations(
            REQUIRED_DATA_MIGRATIONS, print_fn, force_rebuild_all
        )
    ...
    def optimize(
        ...
        self._execute_data_migrations(
            OPTIONAL_DATA_MIGRATIONS, print_fn, force_rebuild_all
        )
    ...
    def has_built_index(self, migration_name: str) -> bool:
        query = (
            ...
            .where(
                SecondaryIndexMigrationTable.c.migration_completed != None
            )  # noqa: E711
            .limit(1)
        )
        with self.connect() as conn:
    ...
    def has_run_stats_index_cols(self) -> bool:
        with self.connect() as conn:
            column_names = [
                x.get("name") for x in db.inspect(conn).get_columns(RunsTable.name)
            ]
    ...
    def has_bulk_actions_selector_cols(self) -> bool:
        with self.connect() as conn:
            column_names = [
                x.get("name")
                for x in db.inspect(conn).get_columns(BulkActionsTable.name)
            ]
    ...
    def replace_job_origin(
        self, run: DagsterRun, job_origin: ExternalJobOrigin
    ) -> None:
