...
from marie._core.events import DagsterEvent
from marie._core.execution.backfill import BulkActionStatus, PartitionBackfill
from marie._core.instance import MayHaveInstanceWeakref, T_DagsterInstance
from marie._core.snap import ExecutionPlanSnapshot, JobSnapshot
from marie._core.storage.marie_run import (
    DagsterRun,
    JobBucket,
    RunPartitionData,
    RunRecord,
...
from marie._core.storage.sql import AlembicVersion
from marie._daemon.types import DaemonHeartbeat
from marie._utils import PrintFn
...
class RunStorage(ABC, MayHaveInstanceWeakref[T_DagsterInstance], DaemonCursorStorage):
    ...
    :py:class:`~marie._core.storage.runs.SqlRunStorage`.
    ...
    def get_runs(
        ...
        Args:
            filters (Optional[RunsFilter]) -- The
                :py:class:`~marie._core.storage.pipeline_run.RunsFilter` by which to filter
                runs
    ...
    def get_runs_count(self, filters: Optional[RunsFilter] = None) -> int:
        ...
        Args:
            filters (Optional[RunsFilter]) -- The
                :py:class:`~marie._core.storage.pipeline_run.PipelineRunFilter` by which to filter
                runs
    ...
    def get_run_groups(
        ...
        Returns:
            Dict[str, Dict[str, Union[List[PipelineRun], int]]]: Specifically, a dict of the form
                ...
                instances of :py:class:`~marie._core.pipeline_run.PipelineRun` returned in this
    ...
    def has_snapshot(self, snapshot_id: str):
        return self.has_job_snapshot(snapshot_id) or self.has_execution_plan_snapshot(
            snapshot_id
        )
