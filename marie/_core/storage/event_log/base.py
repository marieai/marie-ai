...
from marie._core.assets import AssetDetails
from marie._core.definitions.events import AssetKey
from marie._core.event_api import EventHandlerFn, EventLogRecord, EventRecordsFilter
from marie._core.events import DagsterEventType
from marie._core.events.log import EventLogEntry
from marie._core.execution.stats import (
    RunStepKeyStatsSnapshot,
    build_run_stats_from_events,
    build_run_step_stats_from_events,
)
from marie._core.instance import MayHaveInstanceWeakref, T_DagsterInstance
from marie._core.storage.marie_run import DagsterRunStatsSnapshot
from marie._core.storage.sql import AlembicVersion
from marie.utils import PrintFn
...
class AssetEntry(
    ...
    def __new__(
        ...
        # FIXME
        # from marie._core.storage.partition_status_cache import AssetStatusCacheValue
        ...
        return super(AssetEntry, cls).__new__(
            ...
            asset_details=check.opt_inst_param(
                asset_details, "asset_details", AssetDetails
            ),
...
class EventLogStorage(ABC, MayHaveInstanceWeakref[T_DagsterInstance]):
    ...
    def get_step_stats_for_run(
        ...
        if step_keys:
            logs = [
                ...
                and event.get_dagster_event().step_key in step_keys
            ]
    ...
    @abstractmethod
    def reindex_events(
        self, print_fn: Optional[PrintFn] = None, force: bool = False
    ) -> None:
    ...
    def get_asset_keys(
        ...
        if prefix:
            asset_keys = [
                ...
                if asset_key.path[: len(prefix)] == prefix
            ]
        if cursor:
