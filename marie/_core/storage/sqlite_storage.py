...
class DagsterSqliteStorage(DagsterStorage, ConfigurableClass):
    ...
    def __init__(
        self, base_dir: str, inst_data: Optional[ConfigurableClassData] = None
    ):
        ...
        self._schedule_storage = SqliteScheduleStorage.from_local(
            _schedule_directory(base_dir)
        )
    ...
    def event_storage_data(self) -> Optional[ConfigurableClassData]:
        return ConfigurableClassData(
            ...
            yaml.dump(
                {"base_dir": _runs_directory(self.base_dir)}, default_flow_style=False
            ),
        )
    ...
    def run_storage_data(self) -> Optional[ConfigurableClassData]:
        return ConfigurableClassData(
            ...
            "SqliteRunStorage",
            yaml.dump(
                {"base_dir": _event_logs_directory(self.base_dir)},
    ...
    def schedule_storage_data(self) -> Optional[ConfigurableClassData]:
        return ConfigurableClassData(
            ...
            yaml.dump(
                {"base_dir": _schedule_directory(self.base_dir)},
                default_flow_style=False,
            ),
        )
