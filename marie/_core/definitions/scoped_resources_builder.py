...
from marie._core.errors import DagsterUnknownResourceError
...
class ScopedResourcesBuilder(
    NamedTuple(
        ...
        [
            ("resource_instance_dict", Mapping[str, object]),
            ("contains_generator", bool),
        ],
    )
):
    ...
    def build(self, required_resource_keys: Optional[AbstractSet[str]]) -> Resources:
        ...
        # FIXME
        resources_to_attach_to_context = resource_instance_dict
        ...
        if self.contains_generator:
            ...
            class _ScopedResources(
                namedtuple(
                    "_ScopedResources", list(resources_to_attach_to_context.keys())
                ),
                Resources,
            ):
                @property
