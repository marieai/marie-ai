...
# TODO: REMOVE ME
TableRecord: TypeAlias = Mapping[str, Any]
TableSchema: TypeAlias = Mapping[str, Any]
    ...
    for k, v in metadata.items():
        try:
            ...
            if allow_invalid:
                ...
                normalized_value = TextMetadataValue(
                    f"[{v.__class__.__name__}] (unserializable)"
                )
            else:
...
class MetadataValue(ABC, Generic[T_Packable]):
        ...
        Example:
            .. code-block:: python
                ...
                def emit_metadata(context, df):
                    yield AssetMaterialization(
                        asset_key="my_dataset",
                        metadata={"my_text_label": MetadataValue.text("hello")},
                    )
        ...
        Example:
            .. code-block:: python
                ...
                def emit_metadata(context, md_str):
                    yield AssetMaterialization(
                        asset_key="info",
                        metadata={'Details': MetadataValue.md(md_str)},
                    )
    ...
    def python_artifact(python_artifact: Callable) -> "PythonArtifactMetadataValue":
        ...
        return PythonArtifactMetadataValue(
            python_artifact.__module__, python_artifact.__name__
        )
...
class PathMetadataValue(
    NamedTuple("_PathMetadataValue", [("path", PublicAttr[Optional[str]])]),
    MetadataValue[str],
):
...
class NotebookMetadataValue(
    NamedTuple("_NotebookMetadataValue", [("path", PublicAttr[Optional[str]])]),
    MetadataValue[str],
):
...
class IntMetadataValue(
    ...
    def __new__(cls, value: Optional[int]):
        return super(IntMetadataValue, cls).__new__(
...
class BoolMetadataValue(
    ...
    def __new__(cls, value: Optional[bool]):
        return super(BoolMetadataValue, cls).__new__(
            cls, check.opt_bool_param(value, "value")
        )
...
class DagsterRunMetadataValue(
    ...
    def __new__(cls, run_id: str):
        return super(DagsterRunMetadataValue, cls).__new__(
    ...
    def __new__(cls, records: Sequence[TableRecord], schema: Optional[TableSchema]):
        ...
        if len(records) == 0:
            schema = check.not_none(
                schema, "schema must be provided if records is empty"
            )
        else:
            columns = set(records[0].data.keys())
            for record in records[1:]:
                check.invariant(
                    set(record.data.keys()) == columns,
