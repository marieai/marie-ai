...
class AssetKeyDAG(NamedTuple("_AssetKey", [("path", Sequence[str])])):
    ...
    def from_db_string(asset_key_string: Optional[str]) -> Optional["AssetKey"]:
        ...
        if asset_key_string[0] == "[":
            # is a json string
            try:
                path = deserialize_value(asset_key_string)
            except Exception:
...
class AssetLineageInfo(
    NamedTuple(
        "_AssetLineageInfo", [("asset_key", AssetKey), ("partitions", AbstractSet[str])]
    )
):
    def __new__(
        cls, asset_key: AssetKey, partitions: Optional[AbstractSet[str]] = None
    ):
        asset_key = check.inst_param(asset_key, "asset_key", AssetKey)
        partitions = check.opt_set_param(partitions, "partitions", str)
        return super(AssetLineageInfo, cls).__new__(
            cls, asset_key=asset_key, partitions=partitions
        )
...
class AssetMaterialization(
    ...
    def __new__(
        ...
        from marie._core.definitions.multi_dimensional_partitions import (
            MultiPartitionKey,
        )
        ...
        invalid_tags = [
            tag for tag in tags or {} if not tag.startswith(SYSTEM_TAG_PREFIX)
        ]
...
class RetryRequested(Exception):
    ...
    def __init__(
        ...
        self.seconds_to_wait = check.opt_numeric_param(
...
class ObjectStoreOperation(
    ...
    def __new__(
        ...
        return super(ObjectStoreOperation, cls).__new__(
            ...
            object_store_name=check.opt_str_param(
...
class HookExecutionResult(
    ...
    def __new__(cls, hook_name: str, is_skipped: Optional[bool] = None):
        return super(HookExecutionResult, cls).__new__(
            ...
            is_skipped=cast(
                bool, check.opt_bool_param(is_skipped, "is_skipped", default=False)
            ),
        )
