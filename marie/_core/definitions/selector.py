...
from marie._core.definitions.repository_definition import SINGLETON_REPOSITORY_NAME
from marie.serdes import create_snapshot_id, whitelist_for_serdes
...
class JobSelector(
    NamedTuple(
        "_JobSelector",
        [("location_name", str), ("repository_name", str), ("job_name", str)],
    )
):
    def __new__(
        cls,
...
class RepositorySelector(
    NamedTuple(
        "_RepositorySelector", [("location_name", str), ("repository_name", str)]
    )
):
...
class CodeLocationSelector(
    NamedTuple("_CodeLocationSelector", [("location_name", str)])
):
...
class SensorSelector(
    NamedTuple(
        "_SensorSelector",
        [("location_name", str), ("repository_name", str), ("sensor_name", str)],
    )
):
...
class InstigatorSelector(
    NamedTuple(
        "_InstigatorSelector",
        [("location_name", str), ("repository_name", str), ("name", str)],
    )
):
...
class GraphSelector(
    NamedTuple(
        "_GraphSelector",
        [("location_name", str), ("repository_name", str), ("graph_name", str)],
    )
):
...
class PartitionSetSelector(
    ...
    def __new__(cls, location_name: str, repository_name: str, partition_set_name: str):
        return super(PartitionSetSelector, cls).__new__(
            ...
            location_name=check.str_param(location_name, "location_name"),
            repository_name=check.str_param(repository_name, "repository_name"),
            partition_set_name=check.str_param(
