...
from marie._core.instance import DynamicPartitionsStore
...
class PartitionDimensionKey(
    NamedTuple(
        "_PartitionDimensionKey", [("dimension_name", str), ("partition_key", str)]
    )
):
...
def _check_valid_partitions_dimensions(
    ...
    for dim_name, partitions_def in partitions_dimensions.items():
        if not any(
            isinstance(partitions_def, t) for t in ALLOWED_PARTITION_DIMENSION_TYPES
        ):
...
class MultiPartitionsDefinition(PartitionsDefinition[MultiPartitionKey]):
    ...
    def __init__(self, partitions_defs: Mapping[str, PartitionsDefinition]):
        ...
        check.mapping_param(
            ...
            "partitions_defs",
            key_type=str,
            value_type=PartitionsDefinition,
        )
    ...
    def get_partitions_def_for_dimension(
    ...
    def has_partition_key(
        ...
        if partition_key.keys_by_dimension.keys() != set(
            self.partition_dimension_names
        ):
            ...
            multipartition_key = MultiPartitionKey(
                {
                    ...
                    for i, dim in enumerate(self._partitions_defs)
                }
            )
    ...
    def get_tags_for_partition_key(self, partition_key: str) -> Mapping[str, str]:
        partition_key = cast(
            MultiPartitionKey, self.get_partition_key_from_str(partition_key)
        )
    ...
    def time_window_dimension(self) -> PartitionDimensionDefinition:
        ...
        check.invariant(
            len(time_window_dims) == 1,
            "Expected exactly one time window partitioned dimension",
        )
...
def get_tags_from_multi_partition_key(
