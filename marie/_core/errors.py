...
class DagsterRunNotFoundError(DagsterError):
    ...
    def __init__(self, *args, **kwargs):
        self.invalid_run_id = check.str_param(
            kwargs.pop("invalid_run_id"), "invalid_run_id"
        )
...
def user_code_error_boundary(
    ...
    msg_fn: Callable[[], str],
    log_manager: Optional[MarieLogger] = None,
    **kwargs: object,
) -> Iterator[None]:
...
class DagsterUserCodeExecutionError(DagsterError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        user_exception = check.inst_param(
            kwargs.pop("user_exception"), "user_exception", Exception
        )
        original_exc_info = check.tuple_param(
            kwargs.pop("original_exc_info"), "original_exc_info"
        )

        check.invariant(original_exc_info[0] is not None)

        super(DagsterUserCodeExecutionError, self).__init__(
            args[0], *args[1:], **kwargs
        )

        self.user_exception = check.opt_inst_param(
            user_exception, "user_exception", Exception
        )
...
class DagsterSubprocessError(DagsterError):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subprocess_error_infos = check.list_param(
            kwargs.pop("subprocess_error_infos"),
            "subprocess_error_infos",
