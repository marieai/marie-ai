...
from marie._core.definitions.utils import validate_tags
...
class IExecutionStep:
    ...
    def handle(
        self,
    ) -> Union[StepHandle, UnresolvedStepHandle, ResolvedFromDynamicStepHandle]:
        pass
...
class ExecutionStep(
    ...
    def __new__(
        ...
        return super(ExecutionStep, cls).__new__(
            ...
            step_output_dict={
                so.name: so
                for so in check.sequence_param(
...
class UnresolvedMappedExecutionStep(
    NamedTuple(
        ...
        [
            ...
            (
                "step_input_dict",
                Mapping[str, Union[StepInput, UnresolvedMappedStepInput]],
            ),
            ("step_output_dict", Mapping[str, StepOutput]),
            ("tags", Mapping[str, str]),
        ],
        ...
        for inp in self.step_inputs:
            if isinstance(inp, StepInput):
                deps.update(
                    [
                        handle.step_key
                        for handle in inp.get_step_output_handle_dependencies()
                    ]
                )
    ...
    def resolved_by_step_key(self) -> str:
        ...
        check.invariant(
            len(keys) == 1, "Unresolved step expects one and only one dynamic step key"
        )
        return list(keys)[0]
    ...
    def resolved_by_output_name(self) -> str:
        ...
        check.invariant(
            len(keys) == 1,
            "Unresolved step expects one and only one dynamic output name",
        )
    ...
    def resolve(
        ...
        for mapped_key in mapping_keys:
            resolved_inputs = [
                _resolved_input(inp, mapped_key) for inp in self.step_inputs
            ]

            execution_steps.append(
                ExecutionStep(
                    handle=ResolvedFromDynamicStepHandle(
                        self.handle.node_handle, mapped_key
                    ),
...
class UnresolvedCollectExecutionStep(
    NamedTuple(
        ...
        [
            ...
            (
                "step_input_dict",
                Mapping[str, Union[StepInput, UnresolvedCollectStepInput]],
            ),
            ("step_output_dict", Mapping[str, StepOutput]),
            ("tags", Mapping[str, str]),
        ],
        ...
        return super(UnresolvedCollectExecutionStep, cls).__new__(
            ...
            step_input_dict={
                ...
                for si in check.sequence_param(
                    step_inputs,
                    "step_inputs",
                    of_type=(StepInput, UnresolvedCollectStepInput),
                )
            },
            step_output_dict={
                so.name: so
    ...
    def step_input_named(
        self, name: str
    ) -> Union[StepInput, UnresolvedCollectStepInput]:
        check.str_param(name, "name")
        return self.step_input_dict[name]
        ...
        for inp in self.step_inputs:
            if isinstance(inp, StepInput):
                ...
                resolved_inputs.append(
                    inp.resolve(
                        mappings[inp.resolved_by_step_key][inp.resolved_by_output_name]
                    )
                )
