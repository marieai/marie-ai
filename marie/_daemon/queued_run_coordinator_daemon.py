...
from marie._core.events import EngineEventData
from marie._core.instance import DagsterInstance
from marie._core.launcher import LaunchRunContext
from marie._core.run_coordinator.queued_run_coordinator import (
...
from marie._core.storage.tags import PRIORITY_TAG
from marie._core.workspace.context import IWorkspaceProcessContext
from marie._core.workspace.workspace import IWorkspace
from marie._daemon.daemon import DaemonIterator, IntervalDaemon
from marie.utils.error import serializable_error_info_from_exc_info
from marie.utils.tags import TagConcurrencyLimitsCounter
...
class QueuedRunCoordinatorDaemon(IntervalDaemon):
    ...
    def _get_runs_to_dequeue(
        ...
        if not isinstance(instance.run_coordinator, QueuedRunCoordinator):
            check.failed(
                f"Expected QueuedRunCoordinator, got {instance.run_coordinator}"
            )
        ...
        for run in sorted_runs:
            ...
            location_name = (
                run.external_job_origin.location_name
    ...
    def _dequeue_run(
        ...
        try:
            ...
            if run.status not in (DagsterRunStatus.QUEUED, DagsterRunStatus.STARTING):
                ...
                check.invariant(
                    len(enqueue_event_records), "Could not find enqueue event for run"
                )
                ...
                if num_retries_so_far >= run_queue_config.max_user_code_failure_retries:
                    ...
                    retries_left = (
                        run_queue_config.max_user_code_failure_retries
                        - num_retries_so_far
                    )
