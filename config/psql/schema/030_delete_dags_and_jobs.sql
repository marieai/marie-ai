...
-- Usage:
-- Single DAG ID
-- SELECT {schema}.delete_dags_and_jobs(ARRAY['d4f5e6b7-1234-5678-9abc-def012345678'::uuid]);
-- Multiple DAG IDs
-- SELECT {schema}.delete_dags_and_jobs(ARRAY['d4f5e6b7-1234-5678-9abc-def012345678'::uuid, 'a1b2c3d4-5678-9abc-def0-1234567890ab'::uuid]);
-- SELECT {schema}.delete_dags_and_jobs(
--     ARRAY(
...
-- );
 ...
 CREATE OR replace FUNCTION {schema}.delete_dags_and_jobs( p_dag_ids UUID[] ) returns void LANGUAGE plpgsql
AS
  $$
  DECLARE
  ...
  END;
  $$;
  ALTER FUNCTION {schema}.delete_dags_and_jobs(uuid[]) owner TO postgres;