CREATE OR REPLACE FUNCTION {schema}.fetch_next_job(
    job_name   text,
    batch_size integer DEFAULT 250,
    w_hot      numeric DEFAULT 0.70
)
RETURNS SETOF {schema}.job
LANGUAGE sql
STABLE
PARALLEL SAFE
SET work_mem = '256MB'
...
SELECT j.*
FROM picked p
JOIN {schema}.job j ON j.id = p.id
ORDER BY p.job_level DESC, p.priority DESC, j.id;
$$;

ALTER FUNCTION {schema}.fetch_next_job(text, integer, numeric) OWNER TO postgres;
