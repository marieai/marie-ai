...
SELECT
  ...
  dep.depends_on_id,
  dep.depends_on_name AS depends_on_job_name,
  d2.job_level AS depends_on_level,
  d2.state AS depends_on_state,

  d2.started_on AS start_time,
  d2.completed_on AS end_time,
  EXTRACT(EPOCH FROM (d2.completed_on - d2.started_on)) / 60.0 AS duration_mins,
  EXTRACT(EPOCH FROM (d2.completed_on - d2.started_on)) AS duration_sec,
  CASE
