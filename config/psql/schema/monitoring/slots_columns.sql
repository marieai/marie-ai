...
-- This function is generic and accepts any timezone parameter
CREATE OR REPLACE FUNCTION marie_scheduler.day_in_tz(ts timestamptz, tz text)
RETURNS date
LANGUAGE sql
IMMUTABLE
AS $$
...
ALTER TABLE marie_scheduler.job
  ...
  ADD COLUMN IF NOT EXISTS slot_idx15_created  int  GENERATED ALWAYS AS (
      marie_scheduler.slot_15m(created_on, 'UTC')
  ) STORED,
  ADD COLUMN IF NOT EXISTS day_local_created   date GENERATED ALWAYS AS (
      marie_scheduler.day_in_tz(created_on, 'UTC')
  ) STORED,
  ...
  ADD COLUMN IF NOT EXISTS slot_idx15_soft     int  GENERATED ALWAYS AS (
      CASE WHEN soft_sla IS NULL THEN NULL
           ELSE marie_scheduler.slot_15m(soft_sla, 'UTC')
      END
  ) STORED,
  ADD COLUMN IF NOT EXISTS day_local_soft      date GENERATED ALWAYS AS (
      CASE WHEN soft_sla IS NULL THEN NULL
           ELSE marie_scheduler.day_in_tz(soft_sla, 'UTC')
      END
  ) STORED,
  ...
  ADD COLUMN IF NOT EXISTS slot_idx15_hard     int  GENERATED ALWAYS AS (
      CASE WHEN hard_sla IS NULL THEN NULL
           ELSE marie_scheduler.slot_15m(hard_sla, 'UTC')
      END
  ) STORED,
  ADD COLUMN IF NOT EXISTS day_local_hard      date GENERATED ALWAYS AS (
      CASE WHEN hard_sla IS NULL THEN NULL
           ELSE marie_scheduler.day_in_tz(hard_sla, 'UTC')
      END
  ) STORED,
  ...
  ADD COLUMN IF NOT EXISTS slot_idx15_effective int GENERATED ALWAYS AS (
      marie_scheduler.slot_15m(COALESCE(soft_sla, hard_sla, created_on), 'UTC')
  ) STORED,
  ADD COLUMN IF NOT EXISTS day_local_effective date GENERATED ALWAYS AS (
      marie_scheduler.day_in_tz(COALESCE(soft_sla, hard_sla, created_on), 'UTC')
  ) STORED;
...
-- DROP VIEW marie_scheduler.dag_bucketed_sla_view
-- DROP VIEW   marie_scheduler.job_slots_effective
--
