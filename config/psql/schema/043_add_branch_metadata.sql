...
-- Add branch_metadata column to job table if it doesn't exist
DO $$ BEGIN
...
-- Add branch_metadata column to job_history table if it doesn't exist
DO $$ BEGIN
    ALTER TABLE {schema}.job_history ADD COLUMN branch_metadata JSONB;
EXCEPTION
    WHEN duplicate_column THEN NULL;
END $$;

COMMENT ON COLUMN {schema}.job_history.branch_metadata IS 'Stores branch/switch execution metadata for workflow branching';
