...
def test_control_flow_entrypoint_includes_guardrail():
    assert is_control_flow_entrypoint("guardrail://control")
...
def test_frontier_slot_filter_allows_control_flow_and_runnable_executors():
    filt = frontier_slot_filter({"extract_executor": 2, "index_executor": 0})

    assert filt(wi("g1", "guardrail://control"))
