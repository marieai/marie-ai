...
if docarray_v2:
    ...
    class SingleExecutorDeployment(Executor):
        ...
        @requests(on='/foo')
        async def foo(self, docs: DocList[InputTestDoc], **kwargs) -> DocList[OutputTestDoc]:
            for doc in docs:
                doc.text += f'return foo {os.getpid()}'
                doc.tags['pid'] = os.getpid()

        @requests(on='/bar')
        async def bar(self, docs: DocList[InputTestDoc], **kwargs) -> DocList[OutputTestDoc]:
            for doc in docs:
                doc.text += f'return bar {os.getpid()}'
                doc.tags['pid'] = os.getpid()

        @requests(on='/error')
        async def raise_error(self, docs: DocList[InputTestDoc], **kwargs) -> DocList[OutputTestDoc]:
        ...
        async def docs_with_params(self, docs: DocList[InputTestDoc], parameters, **kwargs) -> DocList[OutputTestDoc]:
            for doc in docs:
