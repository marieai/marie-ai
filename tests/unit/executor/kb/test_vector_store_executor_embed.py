...
def _make_executor() -> VectorStoreExecutor:
    ...
    executor._vector_store.add_nodes_batch = AsyncMock(side_effect=lambda nodes, **kw: len(nodes))
    return executor
...
def test_resolve_extraction_output_reads_meta_json_and_flattens_pages(monkeypatch):
    ...
    def fake_read(uri, **kwargs):
        ...
        return json.dumps(
            {
                "ocr": [
                    {"lines": [{"text": "page one line one"}, {"text": "page one line two"}]},
...
async def test_embed_and_store_resolves_asset_key_doc_via_storage_read(monkeypatch):
    ...
    executor._resolve_extraction_output.assert_called_once_with("doc.pdf", "kb_document")
