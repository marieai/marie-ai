...
def test_timer_context(metrics_setup):
    ...
    histogram = meter.create_histogram(name='time_taken', description='measure something')

    with MetricsTimer(summary_metric=summary, histogram=histogram):
        _do_something()

    # Prometheus samples
    summary_count_sample = [sample.value for sample in list(summary._samples()) if '_count' == sample.name]
    assert 1.0 == summary_count_sample[0]
    # OpenTelemetry samples
    histogram_metric = json.loads(metric_reader.get_metrics_data().resource_metrics[0].scope_metrics[0].metrics[0].to_json())
...
def test_timer_decorator(metrics_setup):
    ...
    histogram = meter.create_histogram(name='time_taken_decorator', description='measure something')
