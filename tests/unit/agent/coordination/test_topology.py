...
class TestCoordinationResult:
    ...
    def test_all_succeeded(self):
        ...
        results = [
            AgentResult(agent_name=f"agent_{i}", output=f"output_{i}")
            for i in range(3)
        ]
    ...
    def test_partial_failure(self):
        ...
        results = [
            ...
            AgentResult(agent_name="failure", output=None, status="failed", error="err"),
            AgentResult(agent_name="timeout", output=None, status="timeout", error="to"),
        ]
        coord_result = CoordinationResult(
            results=results,
            merged_output=None,
