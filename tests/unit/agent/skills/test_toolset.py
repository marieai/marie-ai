...
class TestLoadSkill:
    ...
    def test_include_full_content(self, skill_with_instructions):
        ...
        original = dict(SKILL_REGISTRY._skills)
        SKILL_REGISTRY._skills = {"test-skill-with-instructions": skill_with_instructions}

        try:
            result = load_skill(name="test-skill-with-instructions", include_full_content=True)
            data = json.loads(result)
