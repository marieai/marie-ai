...
def test_metadata_envelope_built_from_run_config():
    rr = RunRequest(
        ...
        run_config={
            "uri": "s3://b/tenants/t1/kb-indexes/i1/sources/s1/a.pdf",
            "ref_id": "tenants/t1/kb-indexes/i1/sources/s1/a.pdf",
...
def test_build_work_info_sets_required_scheduler_fields():
    rr = RunRequest(
        run_key="k3", job_name="kb_indexing", priority=5, run_config={"uri": "s3://x/y"}
    )
    before = datetime.now(timezone.utc)
    work_info = build_work_info(
        rr, sensor_id="sid", sensor_name="n", job_name="kb_indexing", dag_id="dag-1"
    )
    ...
    assert work_info.name == "kb_indexing"
