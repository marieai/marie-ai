    ...
    async def setup_server(self):
        ...
        for server in self.servers:
            if isinstance(server, GRPCServer):
                ...
                host = server.host
                port = server.port
                ctrl_address = f'grpc://{host}:{port}'
                await self._register_gateway(ctrl_address)

    async def _register_gateway(self, ctrl_address: str):
        ...
        if False:
            ...
            await asyncio.create_task(_async_wait_all(ctrl_address))
